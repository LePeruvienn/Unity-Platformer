...
function AnimatedPNG(imageName, firstImage, imageCount, delay)
	...
	this.draw = function(delayStart)
		{
		// Output all the images, with display set to 'none', and set a timeout
		// for drawFrame()
		for(i=this.firstImageNumber;i<=this.lastImageNumber;i++) 
		{
		 var html = new Array();
		 html[html.length] = '<img id="' + this.animName + i + '" src=""';
		 if (this.altText)
		 ...
		 //alert(html.join(''));
		 document.getElementById(this.animName+i).src = this.images[i].src;
		}
	...
	this.drawFrame = function()
		...
		if (drawImage)
			...
			// Update display status of each frame
			for(i=this.firstImageNumber;i<=this.lastImageNumber;i++) 
			{
			 if (i!=this.currentImage) 
			  document.getElementById(this.animName+i).style.display = "none";
			}

			// Set the current image frame to be visible
			document.getElementById(this.animName+this.currentImage).style.display = "";
			//alert(this.animName+this.currentImage);
